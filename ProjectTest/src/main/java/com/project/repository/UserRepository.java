...
public class UserRepository implements CrudRepository<User, Integer> {
	...
	public User userExist(User user) {
		...
		user = jdbcTemplate.query("Select * from PROJECTTESTUSER where email_Id = :emailId", args,
				new ResultSetExtractor<User>() {
					...
					public User extractData(ResultSet rs) throws SQLException, DataAccessException {
						...
						while (rs.next()) {
							...
							logger.info(
									"User details are ---------------------" + fname + lname + address + emailId + age);
							user2 = new User();
	...
	public User save(User user) {
		...
		String[] args = { user.getAddress(), user.getAge().toString(), user.getEmailId(), user.getFirstName(),
				user.getLastName(), nextId.toString() };
